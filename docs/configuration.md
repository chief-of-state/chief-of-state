...
| COS_SUBSCRIPTION_ENABLED             | Enable real-time event streaming (Subscribe / SubscribeAll gRPC streams). See [Subscription](./subscription.md).                                                      | false        |
...
| COS_WRITE_SIDE_PROTOCOL              | Protocol for write-side handler: _**grpc**_, _**http**_, or _**both**_ (note: 'both' is not currently supported)                                                      | grpc         |
...
| COS_GRPC_CLIENT_POOL_SIZE            | Number of outbound gRPC channels for write-side. `1` = single channel; `> 1` = channel pool (round-robin) for high concurrency. Reduces "http2 exception" under load. | 1            |
