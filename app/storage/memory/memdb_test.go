...
func TestMemoryJournalStore(t *testing.T) {
	...
	t.Run("testWriteEvents", func(t *testing.T) {
		ctx := context.TODO()
		state, err := anypb.New(new(local.Account))
		assert.NoError(t, err)
		...
		err = store.PersistJournals(ctx, []*local.Journal{journal})
		assert.NoError(t, err)
	...
	t.Run("testReplayEvents", func(t *testing.T) {
		...
		count := 10
		journals := make([]*local.Journal, count)
		for i := 0; i < count; i++ {
			seqNr := i + 1
			journals[i] = &local.Journal{
				PersistenceId:  "persistence-1",
		...
		err = store.PersistJournals(ctx, journals)
		assert.NoError(t, err)

		from := uint64(3)
		to := uint64(6)

		actual, err := store.ReplayJournals(ctx, "persistence-1", from, to, 6)
		assert.NoError(t, err)
